...
1. IG_Empathy (인스타): 공감형, 1인칭 POV, 감성적 단어 사용.
2. YT_Info (유튜브): 정보성, 신뢰감, 매장의 강점과 팩트 중심.
...
st.info("최대 10개의 영상을 한꺼번에 업로드하고 플랫폼별 마케팅 패키지를 만드세요.")

uploaded_files = st.file_uploader("영상 파일을 업로드하세요", type=['mp4', 'mov'], accept_multiple_files=True)

if 'all_results' not in st.session_state:
    st.session_state.all_results = {}

if uploaded_files:
    if st.button(f"🔥 총 {len(uploaded_files)}개 영상 처리 시작"):
        progress_bar = st.progress(0)
        status_area = st.empty()
        
        for idx, file in enumerate(uploaded_files):
            file_name = file.name
            status_area.info(f"⏳ ({idx+1}/{len(uploaded_files)}) '{file_name}' 분석 중...")
            ...
            try:
                ...
                # 3. 모델 호출 [핵심: 캡처본에 있는 2.5-flash 모델 사용!]
    ...
    for file_name, res in st.session_state.all_results.items():
        with st.expander(f"🎬 {file_name} - 플랫폼별 기획안 보기", expanded=True):
            ...
            with col1:
                ...
                st.caption(f"{res['ig']['tags']} | {res['ig']['comment']}")
            ...
            with col2:
                ...
                st.caption(f"{res['yt']['tags']} | {res['yt']['comment']}")
