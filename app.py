...
# ==========================================
# 🔥 새로 추가된 기능: 마케팅 브리프 입력창 (사이드바)
# ==========================================
st.sidebar.header("📝 마케팅 브리프 (영상 기획안)")
st.sidebar.write("AI에게 전달할 핵심 정보를 입력하세요.")

store_name = st.sidebar.text_input("상호명 (예: 동경생고기)", "")
main_menu = st.sidebar.text_input("주력 메뉴 (예: 1++ 한우 특수부위)", "")
key_point = st.sidebar.text_area("강조할 포인트 (예: 단체 회식 환영, 20년 전통, 콜키지 프리)", "")

# 2. 동적 프롬프트 생성 함수 (입력값 반영)
def get_system_prompt(store, menu, point):
    # 입력값이 없으면 기본값 설정
    store_text = store if store else "해당 매장"
    menu_text = menu if menu else "영상 속 주요 상품"
    point_text = f"특별히 강조할 내용: {point}" if point else "영상의 시각적 특징을 잘 살려서 설명할 것"
    
    return f"""
당신은 대한민국 1타 숏폼 마케터입니다. 영상을 보고 **무조건 15초~18초 분량 (말하기 속도 기준 약 60~70자)**의 대본을 쓰세요.
딱딱한 설명은 버리고, 요즘 트렌드에 맞게 재밌고 톡톡 튀는 멘트를 쓰세요.

[🎯 반드시 포함해야 할 핵심 마케팅 정보]
- 상호명: {store_text}
- 주력 메뉴: {menu_text}
- {point_text}

1. IG_Empathy (인스타): 감성적이면서도 위트 있는 1인칭 시점. (예: "아니, {store_text} 사장님 이렇게 퍼주시면...")
2. YT_Info (유튜브): {menu_text}의 팩트를 짚어주되 지루하지 않은 속도감 있는 리뷰톤.

반드시 아래 JSON 형식으로만 답변하세요:
{{
  "ig": {{"script": "15초 분량의 대본", "title": "제목", "tags": "#태그", "comment": "고정댓글"}},
  "yt": {{"script": "15초 분량의 대본", "title": "제목", "tags": "#태그", "comment": "고정댓글"}}
}}
"""
...
uploaded_files = st.file_uploader("영상을 업로드하세요 (초기 테스트는 1개 권장)", type=['mp4', 'mov'], accept_multiple_files=True)

if uploaded_files:
    if st.button("🎬 맞춤형 영상 제작 시작"):
        # 사용자가 입력한 정보를 가져와 프롬프트 완성
        ...
        for idx, file in enumerate(uploaded_files):
            ...
            status_area.info(f"⏳ '{file_name}' 분석 및 편집 중...")
